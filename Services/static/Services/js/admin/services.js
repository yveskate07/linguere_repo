document.addEventListener("DOMContentLoaded", function () {
    ...
    // FIX 5 : le fieldset n'a pas d'id dans ce projet Django.
    // On cherche directement toutes les tr qui contiennent un select field_type,
    // en excluant la ligne template Django (id="fields-empty" / name*="__prefix__")
    function applyToAllRows() {
        document.querySelectorAll('tr').forEach(function (row) {
            // Exclure la ligne template vide de Django (préfixe __prefix__)
            if (row.id === 'fields-empty') return;
    ...
    // MutationObserver : surveiller tous les tbody de la page
    // (le fieldset n'ayant pas d'id, on observe document.body)
    const observer = new MutationObserver(function (mutations) {
        mutations.forEach(function (mutation) {
            mutation.addedNodes.forEach(function (node) {
                if (node.nodeType !== 1) return;
                // Ligne ajoutée directement
                if (node.tagName === 'TR' && node.id !== 'fields-empty') {
                    if (node.querySelector('select[name*="field_type"]')) {
                        applyWidget(node);
                    }
                }
                // Ou tbody/table ajouté contenant des lignes
                node.querySelectorAll && node.querySelectorAll('tr').forEach(function (row) {
                    if (row.id === 'fields-empty') return;
                    if (row.querySelector('select[name*="field_type"]')) {
                        applyWidget(row);
                    }
